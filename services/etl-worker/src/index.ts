...
// ============================================
// DATE CONVERTERS → Moved to helpers/date-converter.ts
// toClickHouseDateTime, convertToClickHouseDateTime, isDateLikeValue
// ============================================
...
// Memory management → moved to helpers/memory-manager.ts
// checkMemory, forceGC
...
// LDAP Schedule kontrolü - Günlük/Saatlik otomatik kullanıcı senkronizasyonu
async function checkLDAPSchedules(): Promise<void> {
  try {
    // LDAP config'lerden zamanı gelen schedule'ları bul
    ...
    for (const config of ldapConfigs) {
      ...
      if (shouldSync) {
        ...
        try {
          // LDAP sync için Redis event gönder (core-service dinliyor)
          ...
          // last_sync_at güncelle (sync başlamadan önce, duplicate önlemek için)
          await db.query(
...
async function checkScheduledJobs(): Promise<void> {
  try {
    ...
    // LDAP schedule'ları da kontrol et
